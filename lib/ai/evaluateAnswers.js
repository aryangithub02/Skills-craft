...
/**
 * Produce per-question scores, feedback, and overall improvement tips for a set of assessment answers.
 *
 * Accepts an array of question objects, filters out unanswered items, and returns a normalized evaluation
 * containing each original question merged with AI-provided scores and feedback (or sensible defaults on failure).
 *
 * @param {Array<Object>} questions - Array of question objects to evaluate.
 *   Each object should include:
 *     - {string} type - The question category or role type.
 *     - {string} question - The question text.
 *     - {string} [userAnswer] - The user's answer text.
 *     - {Array<string>} [expectedKeyPoints] - Key points the answer should cover.
 * @returns {Object} An evaluation summary.
 * @returns {Array<Object>} returns.evaluatedQuestions - Array of the original questions merged with evaluation fields:
 *   - {number} score - Overall score for the answer (0-10).
 *   - {string} feedback - 2-3 sentences of constructive feedback.
 *   - {number} technicalAccuracy - Technical accuracy score (0-10).
 *   - {number} communication - Communication clarity score (0-10).
 * @returns {number} returns.overallScore - Aggregate score for the submission (0-100).
 * @returns {Array<string>} returns.improvementTips - Array of actionable tips to improve overall performance.
 * @throws {Error} If no questions contain a non-empty userAnswer ("No answered questions to evaluate").
 */
