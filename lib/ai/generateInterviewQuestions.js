...
/**
 * Generate a standardized set of 10 multiple-choice interview questions tailored to the provided user profile.
 *
 * Builds a system prompt from the user's industry, domain, optional specificTopic, experience, skills, and interviewType,
 * requests an LLM to produce exactly 10 MCQs with a strict technical/behavioral distribution, extracts and repairs JSON from
 * the model response, and returns a normalized array of question objects.
 *
 * @param {Object} userData - Candidate/profile information used to tailor the questions.
 * @param {string} userData.industry - Target industry (e.g., "Finance", "Healthcare").
 * @param {string} userData.domain - Domain or role focus within the industry (e.g., "Backend Engineering").
 * @param {string} [userData.specificTopic] - Optional narrower technical topic to emphasize for technical questions.
 * @param {number} userData.experience - Years of experience to set difficulty appropriateness.
 * @param {string[]} [userData.skills] - List of skills relevant to the candidate.
 * @param {string[]} [userData.topSkills] - Priority skills to include; combined with `skills`.
 * @param {'technical'|'non-technical'|'mixed'} [userData.interviewType] - Determines distribution of technical vs behavioral questions.
 * @returns {Array<Object>} An array of normalized question objects. Each object follows the schema in the implementation:
 *                         { type, difficulty, question, options, correctAnswer, explanation, userAnswer, score, feedback }.
 * @throws {Error} When the AI returns an empty response, when JSON extraction/parsing fails, or when no valid questions can be produced.
 */
